...
pub fn audit_promotion_candidate(report: &DeepSignalReport) -> PromotionAudit {
    ...
    if let Some(latest) = &report.history.latest {
        let effective_holder_count = report.owners.total_holders.unwrap_or(latest.holder_count);
        if effective_holder_count < 30 {
            blockers.push(format!("holder base is still too fresh: {} holders", effective_holder_count));
        } else if effective_holder_count < 50 {
            caveats.push(format!("holder base is still thin: {} holders", effective_holder_count));
        } else {
            strengths.push(format!("holder base has some breadth: {} holders", effective_holder_count));
        }

        if latest.top_holder_pct >= 20.0 {
            blockers.push(format!("top holder is still too concentrated at {:.2}%", latest.top_holder_pct));
        } else if latest.top_holder_pct >= 15.0 {
            caveats.push(format!("top holder is elevated at {:.2}%", latest.top_holder_pct));
        } else {
            strengths.push(format!("top holder is contained at {:.2}%", latest.top_holder_pct));
        }

        if latest.liquidity_usd < 15_000.0 {
            blockers.push(format!("liquidity is too thin at ${:.0}", latest.liquidity_usd));
        } else if latest.liquidity_usd < 30_000.0 {
            caveats.push(format!("liquidity is still thin at ${:.0}", latest.liquidity_usd));
        } else {
            strengths.push(format!("liquidity is usable at ${:.0}", latest.liquidity_usd));
        }

        let buys = latest.buys_h1.max(0) as f64;
        let sells = latest.sells_h1.max(0) as f64;
        let sell_buy_ratio = sells / buys.max(1.0);
        if sells > buys && sell_buy_ratio >= 1.8 && latest.price_change_h1 <= 12.0 {
            blockers.push(format!(
                "hourly tape is too sell-heavy: {} buys / {} sells with price only {:+.2}%",
                latest.buys_h1, latest.sells_h1, latest.price_change_h1
            ));
        } else if sells > buys {
            caveats.push(format!("hourly tape is sell-heavy: {} buys / {} sells", latest.buys_h1, latest.sells_h1));
        } else {
            strengths.push(format!("hourly tape is not underwater: {} buys / {} sells", latest.buys_h1, latest.sells_h1));
        }
    } else {
        blockers.push("no stored snapshot history exists for this mint".to_string());
    }

    if report.history.snapshots_analyzed < 3 {
        blockers.push(format!("only {} stored snapshots exist — not enough flow history", report.history.snapshots_analyzed));
    } else if report.history.snapshots_analyzed < 6 {
        caveats.push(format!("history is still shallow at {} snapshots", report.history.snapshots_analyzed));
    } else {
        strengths.push(format!("local history covers {} snapshots across {:.1}h", report.history.snapshots_analyzed, report.history.hours_covered));
    }

    if report.history.classification_flips >= 2 {
        caveats.push(format!("classification has flipped {} times ({})", report.history.classification_flips, report.history.classifications_seen.join(", ")));
    }

    if report.owners.creator_controlled_pct <= 0.01 {
        strengths.push("creator is effectively out of the token".to_string());
    }
    if report.owners.amm_controlled_pct >= 20.0 {
        strengths.push(format!("{:.2}% of top-holder exposure is venue-controlled, not wallet-controlled", report.owners.amm_controlled_pct));
    }
    if report.owners.wallet_controlled_pct >= 60.0 {
        caveats.push(format!("wallet-controlled top-holder share is still high at {:.2}%", report.owners.wallet_controlled_pct));
    }
    if report.owners.largest_insider_network_pct_supply.map(|pct| pct >= 20.0).unwrap_or(false) {
        blockers.push(format!("largest insider graph controls {:.2}% of supply", report.owners.largest_insider_network_pct_supply.unwrap_or(0.0)));
    }
    if !report.owners.repeated_balance_clusters.is_empty() {
        caveats.push("top holders contain repeated equal-balance clusters".to_string());
    }

    if report.whales.accumulating_count >= report.whales.distributing_count + 2 && report.whales.net_change_7d_ui > 0.0 {
        strengths.push(format!("whales lean net-accumulative over 7d ({:+.2})", report.whales.net_change_7d_ui));
    } else if report.whales.distributing_count >= report.whales.accumulating_count + 2 && report.whales.net_change_7d_ui < 0.0 {
        blockers.push(format!("whales lean net-distributive over 7d ({:+.2})", report.whales.net_change_7d_ui));
    }

    if let Some(lp) = &report.lp {
        match lp.verdict.as_str() {
            "burnt" | "locked" => strengths.push(format!("LP appears {}", lp.verdict)),
            "held_by_wallet" => blockers.push("LP appears held by a wallet".to_string()),
            _ => caveats.push(format!("LP status unresolved: {}", lp.verdict)),
        }
    }

    for signal in &report.hidden_signals {
        match (signal.direction.as_str(), signal.severity.as_str()) {
            ("bearish", "high") => blockers.push(signal.summary.clone()),
            ("bearish", _) => caveats.push(signal.summary.clone()),
            ("bullish", _) => strengths.push(signal.summary.clone()),
            _ => {}
        }
    }
    ...
    let verdict = if !blockers.is_empty() {
        "reject"
    } else if caveats.len() >= 3 {
        "not_ready"
    } else {
        "plausible"
    };
    let promotable = verdict == "plausible";

    let recommended_note = if promotable {
        let bull = strengths.iter().take(2).cloned().collect::<Vec<_>>().join("; ");
        let risk = caveats.iter().take(2).cloned().collect::<Vec<_>>().join("; ");
        let subject = if symbol.is_empty() {
            format!("{}…{}", &report.mint[..6], &report.mint[report.mint.len() - 4..])
        } else {
            format!("${}", symbol)
        };
        Some(format!(
            "{} thesis: {}. Caveats: {}.",
            subject,
            if bull.is_empty() { "structure is holding".to_string() } else { bull },
            if risk.is_empty() { "none material at audit time".to_string() } else { risk }
        ))
    } else {
        None
    };
