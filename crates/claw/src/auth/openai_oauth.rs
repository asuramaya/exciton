//! OpenAI Codex OAuth — PKCE loopback flow + device-code fallback.
//!
//! Ported from zeroclaw's `auth/openai_oauth.rs` with two changes:
//!   1. `TokenSet` is locally defined here (zeroclaw stored it in a
//!      separate `profiles` module that's not relevant for claw).
//!   2. The "ZeroClaw login complete" success page is rebranded.
//!
...
/// Minimal token shape returned by both the authorization-code and
/// device-code flows. `id_token` carries the JWT we extract the
/// chatgpt-account-id from.
#[derive(Debug, Clone)]
pub struct TokenSet {
...
pub async fn start_device_code_flow(client: &Client) -> Result<DeviceCodeStart> {
    ...
    let response = client
        ...
        .send()
        .await
        .context("start OpenAI device-code flow")?;
    ...
    let parsed: DeviceCodeResponse = response
        .json()
        .await
        .context("parse OpenAI device-code response")?;
    Ok(DeviceCodeStart {
...
pub async fn poll_device_code_tokens(
    ...
    loop {
        ...
        let response = client
            ...
            .send()
            .await
            .context("poll device-code token endpoint")?;
        if response.status().is_success() {
        ...
        if let Ok(err) = serde_json::from_str::<OAuthErrorResponse>(&text) {
            match err.error.as_str() {
                ...
                "access_denied" => anyhow::bail!("device-code authorization denied"),
                "expired_token" => anyhow::bail!("device-code expired"),
                _ => anyhow::bail!(
                    "device-code polling failed ({status}): {}",
                    err.error_description.unwrap_or(err.error)
                ),
            }
        }
        anyhow::bail!("device-code polling failed ({status}): {text}");
    }
}
...
pub async fn receive_loopback_code(expected_state: &str, timeout: Duration) -> Result<String> {
    let listener = TcpListener::bind("127.0.0.1:1455")
        .await
        .context("bind callback listener at 127.0.0.1:1455")?;
    ...
    let body =
        "<html><body><h2>claw login complete</h2><p>You can close this tab.</p></body></html>";
    let response = format!(
...
async fn parse_token_response(response: reqwest::Response) -> Result<TokenSet> {
    if !response.status().is_success() {
        let status = response.status();
        let body = response.text().await.unwrap_or_default();
        anyhow::bail!("OpenAI token request failed ({status}): {body}");
    }
...
pub use crate::auth::oauth_common::generate_pkce_state as new_pkce_state;
